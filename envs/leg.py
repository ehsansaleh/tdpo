...
class SimInterface():
    ...
    def step_outer(self, a):
        ...
        return s, r*self.options['reward_scaling'], done
        ...
        if self.options['stand_reward'] == 'simplified':
            ...
            if self.options['add_act_sm_r']:
                ...
                if self.inner_step_count == 0:
                    self.traj_tau_hip = np.zeros((done_inner_steps+1), np.float64)
                    self.traj_tau_knee = np.zeros((done_inner_steps+1), np.float64)
                    self.r_overshoot_applied = False
                self.traj_tau_hip[1+self.inner_step_count] = tau_hip
                self.traj_tau_knee[1+self.inner_step_count] = tau_knee
...
def simulate_and_plot(sim_interface, policy, fig_width=6,
    ...
    if traj['hip_pos'].shape[0] > (sim_s0+4000):
