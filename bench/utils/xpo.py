...
class Actor(torch.nn.Module):
    def __init__(self, observation_dim, action_dim, **kwargs):
        ...
        if 'output_scale' in kwargs:
            output_scale = kwargs.pop('output_scale')
        else:
            output_scale = kwargs.pop('mlp_output_scaling', 1.)
        if 'output_activation' in kwargs:
            output_activation = kwargs.pop('output_activation')
        else:
            do_mlp_output_tanh = kwargs.pop('do_mlp_output_tanh', False)
            output_activation = None if do_mlp_output_tanh else torch.tanh
        activation = kwargs.pop('activation', 'tanh')
        assert activation == 'tanh'

        self.hidden_units = kwargs.pop('layers', [64, 64])
        self.output_scale = output_scale
        self.output_activation = output_activation
        self.use_bias = kwargs.pop('use_bias', True)
        self.fixed_std = kwargs.pop('actor_fixed_std', None)
        assert len(kwargs) == 0, f'Unknown actor options {kwargs}'
...
class RBFActor(torch.nn.Module):
    def __init__(self, observation_dim, action_dim, **kwargs):
        ...
        kwargs = dict(kwargs)
        self.rbf_units = kwargs.pop('rbf_units', 200)
        self.output_scale = kwargs.pop('output_scale', 1.)
        self.output_activation = kwargs.pop('output_activation', None)
        self.use_bias = kwargs.pop('use_bias', False)
        self.initial_rbf_std = kwargs.pop('initial_rbf_std', 1.)
        self.use_independent_rbf_std_per_unit = kwargs.pop('use_independent_rbf_std_per_unit', False)
        self.fixed_inner_rbf_std = kwargs.pop('fixed_inner_rbf_std', True)
        self.fixed_std = kwargs.get('actor_fixed_std', 0.)
        assert len(kwargs) == 0, f'Unknown actor options {kwargs}'
        ...
        if identity_stabilization is not None:
            if isinstance(vec, tuple):
                for x_, y_ in zip(JtMJV, vec):
                    x_.data.add_(y_, alpha=identity_stabilization)
            else:
                offset = 0
                for x_ in JtMJV:
                    numel = x_.numel()
...
class PPOAgent(object):
    def __init__(self, env, **kwargs):
        ...
        self.actor_class = kwargs.pop('actor_class', 'mlp')
        self.net_init_f = kwargs.pop('net_init_f', None)
        self.actor_out_initgain = kwargs.pop('actor_out_initgain', 0.001)
        self.kwargs = kwargs
        self.actor = None
        self.critic = None
        self.archive = None
    ...
    def reset(self, **kwargs):
        ...
        for name, p in self.actor.named_parameters():
            if self.net_init_f is None:
                if 'weight' in name:
                    p_gain = self.actor_out_initgain if 'mu' in name else 1.
                    torch.nn.init.xavier_uniform_(p, gain=p_gain)
                elif 'bias' in name:
    ...
    def append_to_archive(self, file_name, file_like_obj):
    ...
    def save(self, filename):
        ...
        torch.save(self.actor.state_dict(), file_like_obj)
        file_like_obj.seek(0)
        self.append_to_archive(filename, file_like_obj)
    ...
    def close(self):
        ...
        if (mpi_rank == 0) and (self.archive is not None):
            self.archive.close()
    ...
    def env_rollout_vine(self, n_steps, gamma, policy, explorer, d1=1, sample_num=10,
        ...
        if rank == 0:
            ...
            samples_count_ = gathered_data['done_steps'].sum().item()
            samples_count_ += gathered_data['done_steps_vine'].sum().item()
            out_data['samples_count'] = samples_count_
    ...
    def learn(self, nondefault_options={}):
        ######################################################################
        # options handling
        ...
        options = {
            ...
            'vine_params': None,
            # 'vine_params' : dict(n_vine_samples = 10, action_disturbance = 0.01,
        ...
        if alg_ == 'wtrpo':
            C_w2 = options['wtrpo_opt_params'].get('C_w2', None)
            C_wg = options['wtrpo_opt_params'].get('C_wg', None)
        ...
        vine_sample_num = options['vine_params'].get('n_vine_samples', 10)
        ...
        if options['enable_tb'] and rank == 0:
            ...
            self.tar_path = os.path.join(logdir, 'checkpoints.tar')
        ...
        time_of_last_snapshot = time.time()
        number_of_iterations = options['total_timesteps'] // options['steps_per_iteration_per_process'] // size
        ...
        iter = -1
        while n_steps <= options['total_timesteps']:
            iter = iter + 1
            ...
            if alg_ == 'ppo':
                ...
                if line_search_coeffs is not None:
                    ...
                    for _, (ls_c_, ls_seed_) in enumerate(zip(my_ls_coeffs, my_ls_seeds)):
                        ...
                        for p_, backup_p_, payoff_dir_p_ in zip(self.actor.parameters(),
                            ...
                            torch.add(backup_p_.data, payoff_dir_p_.data,
                                      alpha=ls_alpha_, out=p_.data)
                        ...
                        if self.env_type == 'stepper':
                            ...
                            ls_timesteps = greedy_lite_outdict['done_steps'].item()
                        else:
                            raise Exception(f'Unknown env_type {self.env_type}')

                        my_ls_payoffs.append(traj_payoff)
                        my_ls_timesteps.append(ls_timesteps)

                    # Gathering the results
                    ls_payoffs_full = gather_np(np.array(my_ls_payoffs).reshape(1,-1))
                    #ls_delta_steps = gather_np(np.array(my_ls_timesteps).reshape(1,-1)).sum().item()
                    #ls_delta_steps = comm.bcast(delta_steps, root=0)
                    ls_delta_steps = comm.allreduce(sum(my_ls_timesteps), op=MPI.SUM)
                    n_steps += ls_delta_steps
                    steps_since_last_snapshot += ls_delta_steps
                ...
                if rank == 0:
                    ...
                    w2_val = payoff_unit_wasssq_dist_ * (best_alpha ** 2)
                    if not torch.is_tensor(w2_val):
                        w2_val = torch.tensor(w2_val)
                    w2_val = w2_val.double()
                    w2_term = C_w2 * w2_val / 2.
                    gain_term = payoff_unit_gainsq_dist_ * (best_alpha ** 2)
                    if not torch.is_tensor(gain_term):
                        gain_term = torch.tensor(gain_term)
                    gain_term = C_w2 * gain_term.double() / 2.
                    ...
                    for p_, backup_p_, payoff_dir_p_ in zip(self.actor.parameters(),
                                                            backup_actor.parameters(),
                                                            payoff_inv_dir):
                        torch.add(backup_p_.data, payoff_dir_p_.data, alpha=best_alpha, out=p_.data)
            ...
            if rank == 0 and options['enable_tb']:
                ...
                snapshot_step_interval = options['total_timesteps'] // options['number_of_snapshots']
                snapshot_time_interval = options['walltime_hrs'] / options['number_of_snapshots']
                if ((steps_since_last_snapshot >= snapshot_step_interval) or
                    ((time.time() - time_of_last_snapshot) >= snapshot_time_interval)):
                    steps_since_last_snapshot = 0
                    time_of_last_snapshot = time.time()

                    # save actor
                    self.save(f'actor_{n_steps}.pth')
